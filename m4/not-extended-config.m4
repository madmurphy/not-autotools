...
dnl  and paste it in your `configure.ac` before getting started. Furthermore,
dnl  this document will need to be to be included in its entirety.
dnl
...
dnl      NC_THREATEN_FILES([...])
dnl
dnl  Alternatively you can use `NR_THREATEN_BLINDLY` for automatically adding
dnl  all the files present in the templates subdirectory. `NR_THREATEN_BLINDLY`
...
dnl  or `NR_THREATEN_BLINDLY` within conditionals evaluated at `configure` time
...
dnl          [NC_THREATEN_FILES([package.json])])
dnl
dnl  No matter whether you use `NC_THREATEN_FILES()` or `NR_THREATEN_BLINDLY`,
...
dnl  `NC_THREATEN_FILES()` or `NR_THREATEN_BLINDLY` are invoked, the
...
dnl  - `NR_THREATEN_BLINDLY`: recursively registers all the templates in
